...
with tab4:
   ...
   if not tabla_mes.empty:
        ...
        st.write("### Relación Irradiancia vs Temperatura")
        fig, ax = plt.subplots()
        ax.scatter(tabla_mes['Irradiancia (W/m²)'], tabla_mes['Temperatura (°C)'], color="purple")
        ax.set_xlabel("Irradiancia (W/m²)")
        ax.set_ylabel("Temperatura (°C)")
        ax.set_title(f"Relación Irradiancia vs Temperatura - {nombre_mes} 2019")
        st.pyplot(fig)

        # Gráfico de barras (Distribución de temperatura)
        st.write("### Distribución de Temperatura")
        fig, ax = plt.subplots()
        ax.hist(tabla_mes['Temperatura (°C)'], bins=10, color="grey", edgecolor="black")
        ax.set_title(f"Distribución de Temperatura - {nombre_mes} 2019")
        ax.set_xlabel("Temperatura (°C)")
        ax.set_ylabel("Frecuencia")
        st.pyplot(fig)
   else:
        st.error(f"No hay datos disponibles para el mes seleccionado: {nombre_mes}.")
